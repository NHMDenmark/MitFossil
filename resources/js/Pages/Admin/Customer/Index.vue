...
<template>
    <Head :title="$t('layout.admin.customer_text')" />

    <AdminLayout>
        <div class="flex-content container-fluid p-3 p-md-45 bg-white-3">
            <CardHeader :title="$t('layout.admin.customer_text')"></CardHeader>

            <Table class="mt-3" :header="table" :items="users" paginate>
                <template #buttons="{ id, item }">
                    <PrimaryButton :url="route('admin.customer.edit', id)"> {{ $t('form.edit') }}  </PrimaryButton>
